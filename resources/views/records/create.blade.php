    ...
    <div class="max-w-[50rem] mx-auto">
        <form action="/records" method="POST" enctype="multipart/form-data">
            ...
            <div class="pt-3 pb-1 px-3">
                ...
                <input type="text" name="record[title]" placeholder="ここにタイトルを入力" value="{{ old('record.title') }}" class="w-full"/>
                <p class="title__error" style="color:red">{{ $errors->first('record.title') }}</p>
            </div>
            
            <div class="pl-3 py-1">
                <h2>棋譜ファイル<span class="text-red-600">（必須）</span></h2>
            ...
            <div class="px-3 py-1 flex flex-wrap">
                <div class="w-1/2 min-w-80">
                    <h2>先手の名前</h2>
                    <input type="text" name="record[first_player_name]" placeholder="ここに先手の名前を入力" value="{{ old('record.first_player_name') }}" class="w-full"/>
                    <p class="first_player_name__error" style="color:red">{{ $errors->first('record.first_player_name') }}</p>
                </div>
                
                <div class="w-1/2 min-w-80">
                    <h2>後手の名前</h2>
                    <input type="text" name="record[second_player_name]" placeholder="ここに後手の名前を入力" value="{{ old('record.second_player_name') }}" class="w-full"/>
            ...
            <div class="px-3 py-1 flex flex-wrap">
                <div class="w-1/2 min-w-80">
                    <h2>先手の戦法</h2>
                    <input type="text" name="record[first_player_strategy]" placeholder="ここに先手の戦法を入力" value="{{ old('record.first_player_strategy') }}" class="w-full"/>
                    <p class="first_player_strategy__error" style="color:red">{{ $errors->first('record.first_player_strategy') }}</p>
                </div>
                <div class="w-1/2 min-w-80">
                    <h2>後手の戦法</h2>
                    <input type="text" name="record[second_player_strategy]" placeholder="ここに後手の戦法を入力" value="{{ old('record.second_player_strategy') }}" class="w-full"/>
            ...
            <div class="px-3 py-1 flex flex-wrap">
                <div class="w-1/2 min-w-80">
                    <h2>先手の囲い</h2>
                    <input type="text" name="record[first_player_castle]" placeholder="ここに先手の囲いを入力" value="{{ old('record.first_player_castle') }}" class="w-full"/>
                    <p class="first_player_castle__error" style="color:red">{{ $errors->first('record.first_player_castle') }}</p>
                </div>
                <div class="w-1/2 min-w-80">
                    <h2>後手の囲い</h2>
                    <input type="text" name="record[second_player_castle]" placeholder="ここに後手の囲いを入力" value="{{ old('record.second_player_castle') }}" class="w-full"/>
            ...
            <div class="px-3 py-1">
                <h2>詳しい説明</h2>
                <textarea name="record[remark]" style="field-sizing: content;" placeholder="ここに説明を入力" class="w-full">{{ old('record.remark') }}</textarea>
            ...
            <div class="flex justify-end mb-3">
                <input type="submit" class="bg-[#0636C5] hover:bg-[#0636C5] hover:opacity-50 text-white rounded px-3 py-1 mr-3" value="投稿"/>
            </div>
        </form>
    </div>
        {{--<div class="footer">
